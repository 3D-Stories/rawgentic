...
#: Tokens are self-generated by begin_preflight ("pf-" + 16 hex chars) and so never
#: attacker-controlled THERE, but read_preflight/record_preflight_answer/
#: abandon_preflight all accept a CALLER-SUPPLIED token — Step 8a cross-model review,
#: High finding 4: joining it into a path unvalidated let a crafted token escape
#: claude_docs/.supervision-preflight/ (mirrors supervision_lib._CAMPAIGN_ID_RE's own
#: path-safety convention, and the same class of fix already applied to
#: mark_transport_verified's evidence token in this same review pass).
...
def preflight_path(workspace_root: str, token: str) -> str:
    if not isinstance(token, str) or not _TOKEN_RE.match(token):
        raise PreflightError(f"token is not a safe path component: {token!r}")
