...
def any_high_risk_path(paths, extra_patterns=()) -> str | None:
    ...
    (not the matched pattern), or None if no path matches.
    """
    for p in paths:
        if _path_matches_high_risk(p, extra_patterns):
            return p
    return None
...
def should_run_diff_review(
    ...
    Pure (no I/O) so the 9-cell decision matrix is unit-testable. Evaluated in
    order:
