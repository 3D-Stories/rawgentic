...
**Status:** all four children MERGED (2026-07-25). The epic issue stays **OPEN**, tracking the owner-side UAT below — **UAT-3 is the one that matters**.
...
### UAT-5 — #639's regression check does what it claims ✅ evidence gathered, yours to confirm
**Do:** run `python3 tests/phase_executor/live/herdr_ac1_protocol.py --gate` from a session inside
a herdr pane, then again with herdr removed from PATH.
**Pass:** the live run does 20 cold + 20 reused reps with 0 failures per condition; the
herdr-absent run **refuses to report success** rather than failing or silently passing.
**Already observed (2026-07-25, herdr 0.7.5, from pane `w1:p1`):** the gate printed
`GO (cold: 20 reps, 0 identity, 0 env, reused: 20 reps, 0 identity, 0 env)` and exited 0; with herdr
off PATH it exited **4** printing "The gate did NOT run — this is not a pass"; with `RUN_LIVE` unset
the pytest form gave 3 visible SKIPs. This is the one UAT item already evidenced end-to-end — it
wants your eyes on the result, not a fresh run.
