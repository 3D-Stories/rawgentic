...
## Problem
Unattended/overnight harness runs (`claude -p --resume` sessions) can text the owner (outbound, `sentinel/bin/notify.sh`) but cannot receive the **reply**. Phase 1 closes the inbound half so a run can ask by text and receive the answer by text.
...
- **Transport (accepted LAN exception):** `message/query` and `message/text` go over plaintext HTTP on the homelab LAN (`.205`↔`.148`, same-L2). Password + message contents are exposed to an on-path LAN observer. Accepted threat-model exception — consistent with the existing production `notify.sh` (same plaintext `http://` bridge); the endpoint is LAN-internal, the credential least-privilege. HTTPS/tunnel is out of Phase-1 scope (would touch the shared BlueBubbles server config).
