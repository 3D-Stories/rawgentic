...
6. **(F5) Injectability — normative:** `ask_owner` gains `transport=None, sleep=None, clock=None` params (repo convention: all I/O injected); injected paths short-circuit the retry (no real sleeps in tests). Tradeoff recorded: the post-send self-query is racy vs capturing the guid from the send response, chosen to avoid touching sentinel-shared notify.sh (cross-project blast radius); the racy path fail-opens to `sent_guid: null` (token fallback), never wrong-acts.
