...
- **Session model choice changes for Fable users [C].** Through 2026-07-07 Fable 5 draws from weekly limits (at 50%); **from 2026-07-08 every Fable token bills as usage credits at API rates ($10/$50) even on Max** — it leaves the bucket entirely. Recommendation for subscribers: **Opus 4.8 (in-plan) as the orchestrator/session model**, Fable reserved for deliberately credit-funded engagements. rawgentic's `modelRouting` already targets opus/sonnet, so no config change — this is session-model guidance for the docs.
