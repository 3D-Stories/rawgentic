...
### Subscription economics (Pro/Max) — the majority-user case

More than half of rawgentic's users are expected to be **subscription** users (Pro/Max), where the currency is not dollars but **shared plan bucket**: one allowance across Claude Code + Chat + Cowork, metered per 5-hour rolling window plus a weekly cap, with **hard lockout** at the cap (no automatic pay-as-you-go; usage-credit overflow is an explicit opt-in billed at full API rates) [C: support.claude.com/en/articles/11145838]. Exact quotas are unpublished — only the 1×/5×/20× multipliers are official [C]. What carries over, what changes:

- **The relative math carries over intact.** Bucket drain is proportional to model cost, and **cache reads count against the bucket at the same 0.1× discount** — Claude Code even auto-requests 1-hour cache TTL on subscription at no extra cost [C: code.claude.com/docs/en/prompt-caching]. So the ~1.5× cached strong-top premium is the same premium in bucket units. The verdict is unchanged in structure.
- **The failure cost gets sharper, which strengthens the verdict.** On API billing a loop-back costs ~$2.30; on subscription it can push the session over the window/weekly cap and **stall the whole campaign until reset** — hours of wall-clock for an overnight run, and the weekly bucket it burns is shared with everything else the user does that week. Rework variance, not average spend, is what busts a cap; the strong orchestrator is precisely the rework minimizer. And within an unfilled bucket, the strong orchestrator's premium costs **zero marginal dollars** — cheap-top "savings" buy nothing until you are cap-constrained.
- **Session-model choice: the rule is "strongest model your billing mode keeps in-plan (or deliberately funded) at the orchestrator slot" — not "Opus" per se.** Opus 4.8 is recommended today only because it is the strongest *in-plan* model for subscribers [C: through 2026-07-07 Fable 5 draws weekly limits at 50%; from 2026-07-08 every Fable token bills as usage credits at API rates ($10/$50) even on Max]. Three cases, future-proofed:
  1. **Today, routine runs:** Opus 4.8 in-plan on top. Recorded loop-back rate at Opus-tier orchestration is already 0 — paying extra for delta on a zero failure rate has no measurable return until the run-records say otherwise.
  2. **Credits-funded Fable (available now):** a subscriber who enables usage credits gets an attractive hybrid — Fable prices are exactly 2× Opus (incl. cache reads $1 vs $0.50), so the cache-aware orchestrator slot costs ≈ **$5.50/run in credits** while every down-routed worker stays in-plan at zero marginal dollars. *Dollars buy the judgment tier; the bucket pays the volume tier* — and the Fable orchestrator's tokens don't drain the bucket at all, saving window headroom. Worth it for high-stakes engagements (architecture reviews, campaign orchestration, hard debugging — this review itself is the pattern); not for routine WF2 runs per case 1.
  3. **If Fable returns in-plan (plausible — the current credits-only status reads as launch gating, not permanent policy [I]):** the same rule promotes Fable to the top slot by default. Caveat for cap-constrained users: Fable burns bucket ~2× Opus, so the question becomes whether its orchestration delta further reduces rework — unmeasured today [I]; the AC9 substrate answers it (loop-backs and cost by session model in run-records) within a few campaigns.
  rawgentic's `modelRouting` (opus/sonnet roles) is unchanged in every case — this is session-model guidance for the docs, and it should be written as the rule above so it survives model-lineup changes.
- **Effort routing matters *more*, not less:** thinking tokens count against the bucket at full weight [C: platform.claude.com extended-thinking] — low/medium effort on mechanical subagents is direct window stretch.
- **/fast is never in-plan** — usage credits only, from the first token [C: code.claude.com/docs/en/fast-mode]. Interactive-and-deliberate only, for subscribers doubly so.
- **Headless runs on subscription auth [C: code.claude.com/docs/en/authentication]:** `claude setup-token` issues a one-year OAuth token officially supported for CI/GitHub Actions (`CLAUDE_CODE_OAUTH_TOKEN`); bare mode (`--bare`) is the exception (API key only). Consequence for AC8e/M4: the Action pilot must pick an auth mode — **subscription OAuth** (campaign shares the owner's bucket; overnight scheduling exploits windows the human doesn't use, and the driver must map a rate-limit lockout to a `budget` DEFER and resume after window reset) vs **API key** (isolated dollar budget, the AC1 math applies directly). Default recommendation: subscription OAuth with lockout-aware DEFER, since that is the majority case; API key for teams that want campaign spend isolated.
- **AC9 consequence:** for subscribers, `cost_estimate_usd` is virtual — record **tokens by model** (the bucket proxy; ccusage reports exactly this) as the primary metric, dollars as derived.
- Batch API discounts are irrelevant to Claude Code sessions on any billing mode [C: pricing doc — "no batch mode" for sessions].
...
**Campaign anchor: an epic issue — auto-offered, required only headless.** Should a multi-issue run *require* an epic (a GitHub issue task-listing the children)? Verdict: **optional interactively, mandatory for headless runs.**
- *For it:* the epic is GitHub-native visibility (task-list checkboxes, tracked-by relationships), the natural home for the driver's STATUS/QUESTION comments (#48's pattern — headless runs need that channel to exist), a ready-made DAG input (task-list order + child `depends on #N`), and a clean referent for the campaign-level goal ("all children of #N closed or deferred with comment").
- *Against requiring it everywhere:* ceremony for a 2-issue ad-hoc run, and a second source of truth that can drift from the queue state file.
- *Resolution:* the **state file stays the machine source of truth** (statuses, anchors, deferrals); the epic is the human-facing intent + progress mirror. The driver updates epic checkboxes *from* the state file (one-way mirror) and never reads status back from checkboxes. Driver accepts either an epic number (derives the queue from its task list) or an inline issue list (offers to create the epic — one `gh` call). Headless mode refuses to start without one, because the epic is where an unattended run talks to its humans.
...
- **Absolute subscription quotas** — Anthropic publishes only the 1×/5×/20× multipliers, not tokens/hours per window [C that they're unpublished]; the subscription analysis therefore reasons in ratios and lockout mechanics, never absolute headroom. Third-party "tokens per window" figures circulating online are fabricated.
