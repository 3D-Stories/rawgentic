...
- **AC-B4** — Every executor instance is **cost-guarded by pre-launch reservation**: an atomic reservation against the run + quota-pool budgets BEFORE launch, plus a hard per-invocation cap where the provider supports one (claude: `--max-budget-usd`, F2). Providers without a hard cap must declare a compensating enforceable bound in the manifest — **as amended 2026-07-21 (owner decision D-13, #558):** zhipu's bound is the request-level `max_tokens` (`bounds.max_tokens`, adapter-enforced); codex's bound is **the enforced supervisor/`run_subprocess` timeout** — `codex exec` exposes no token/turn option (live `--help` probe, #558 Step 2), so the original "timeout + max-token/turn limits" clause is unimplementable there; token/turn caps adopt if a future codex CLI grows the flag. Post-hoc Observation accounting alone is NOT a guard (gpt review #3). No unbounded fan-out. *(#558 shipped the per-call cap wiring: `bounds.max_budget_usd` on every claude-bearing seat, `bounds.max_tokens` schema+adapter, min-timeout composition on both dispatch paths. The pre-launch atomic run-level reservation clause remains open — epic-#560-adjacent follow-up.)*
...
- **U-3** — Utility fan-out: **RESOLVED in §5b (supersedes the earlier assumption here — both reviewers flagged the stale text, gpt #5 / glm #1):** ALL model-based dispatch — named phase seats AND WF2 Step 2-style utility fan-out — routes through executor jobs; pure deterministic orchestration logic stays in-process; after the run-level architecture flip **no second model-dispatch mechanism remains**. Each run's architecture version declares and enforces exactly one policy. *Owner-RATIFIED 2026-07-22 (the #474 flip decision, verbatim in mempalace `rawgentic/decisions`):
flip the default to executor everywhere, run-level enforced, no runtime auto-fallback, rollback =
a deliberate joint config change — with ONE deviation from the W12 issue text: the legacy
Agent-tool path + bundled agent definitions are KEPT in-tree as the manual rollback target
(never deleted). Shipped by #474 (v3.93.0): `defaultArchitecture` workspace key (absent =
executor), `begin-run` run-start declaration + ledger pin, entry-point enforcement,
architecture-conditioned prose. The mechanical Agent-tool interceptor is #606.*
