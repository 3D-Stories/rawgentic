...
**Draft issue J — `feat(driver): dependency DAG on the #148 queue (depends on #148)`** *(comment/extend #148 first; file only what #148's build doesn't absorb)*
- ACs: (1) Queue schema v2: `issues[].depends_on[]` parsed from issue bodies (`depends on #N`, `blocked by #N`, task-list refs) + `gh api` issue relationships. (2) Topo-sort at campaign start; **cycles halt fail-closed with the cycle printed**. (3) Advance rule: first queued issue whose deps are `merged` (policy knob `deps_satisfied_by: merged|pr_open`, default merged). (4) Deferred dependency parks dependents (`cross-issue-dependency` reason); loop continues with independent issues. (5) v1 state files still readable.
- Risk: medium. Size: **M**. Metric: 5-issue dependency-ordered campaign, 0 ordering violations.
