...
- Runtime environment access (SSH to darwin and my-api-dev for immediate diagnosis)
...
1. Check engine status: `curl -H "X-API-Key: $KEY" http://192.0.2.12:8080/status`
2. Check IBKR connection: engine logs for "connected"/"disconnected"
3. Check scheduler: `curl -H "X-API-Key: $KEY" http://192.0.2.12:8080/scheduler`
