...
7. **Persist `teardown_phase: "clearing"` BEFORE sending anything**, then `send-text "/goal clear"` + `send-keys Enter` with **both** return codes checked, then poll the predecessor's transcript below a baseline taken before the send for a `met:true, sentinel:true` row. A zero return code proves keystrokes were transported, not that the slash command was parsed — without the semantic confirmation a silently ignored `/goal clear` reaches close-before-clear with every other check green. On timeout: `clear_unconfirmed`, and the pane is left **OPEN**.
8. **Close** with two bounded retries, then clear the phase.
