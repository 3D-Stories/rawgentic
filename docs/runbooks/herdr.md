...
| 4 | `goal_armed` | successor transcript: a `goal_status` attachment with `met:false` whose condition is the one actually armed | predecessor |
| 5 | `prompt_landed` | successor transcript: the generation-bound handoff marker, matched as a plain **substring** | predecessor |
