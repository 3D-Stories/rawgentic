...
11. **A bounded SETTLE, which gates nothing** (PR 992). Sleep `SETTLE_BEFORE_GOAL_S`, then ask `herdr agent wait <new> --until idle` with a short budget and treat the answer as **advice**. It is recorded either way, so an operator can tell a healthy handoff from a lucky one. It is not a gate and it must never become one — the idle signal hangs on panes that are already idle.
12. **SEND 3 — the guard, last.** `herdr pane send-text <new> "/goal <condition>"` then `send-keys Enter`.
13. **Poll the transcript until the guard is proven armed** — a `goal_status` row with `met: false` whose condition is the one just armed.
...
#### 7.1.1 Why the bind is its own send and `/goal` goes last (#694; #989's reversal was reverted)

Two things changed here, and the second one has now reversed three times. The current answer is #694's.
...
**`/goal` goes last, and this is measured.** A `/goal` pasted into a session *actively mid-turn* on 2026-07-29 produced its `goal_status met:false` row **while that turn was still running**, so it needs no idle window. The old ordering armed the guard first on the reasoning that work handed to an unguarded session is an unguarded run. That concern is **answered rather than discarded**: the predecessor is not retired until the last rung passes, so "work begins unguarded" never coincides with "the predecessor is already gone" — which was the actual harm. The residual unguarded window is between send 2 and step 13, bounded by exactly the thing that closes it.

**#989 moved `/goal` SECOND and was REVERTED. Read this before moving it again.**

#989 argued that a **bare slash command** pasted into a **busy** session is **QUEUED, not executed** — measured by #718, and enforced by `validate_inserted_prompt`, which refuses to send one. Because the order above puts `/goal` last *deliberately* while the successor is mid-turn on the prompt, #989 concluded it was queued every time, and cited four consecutive failures at `goal_armed` on 2026-08-07 whose transcripts showed `queue-operation enqueue`, a `remove` 2.3 s later, and zero `goal_status` rows. It gave the guard an idle window and handed the busy window to the prompt.

**Two facts refuted it.** First, the owner measured this page's own order — `/goal` last, into a busy pane, every time — running **reliably for weeks**. So being queued is not by itself fatal: a queued command executes once the turn ends. Second, the four failures had a different cause. A goal condition ending in a newline made the paste land in the input box and never submit; PR 991 fixed that at the goal choke point, and three live handoffs isolated it (the same 557-character condition failed twice from a file and armed first time inline).

**What survives from #989.** Nothing in the ordering. Its two follow-ups do survive: PR 991's trailing-line-ending strip, and PR 992's settle (step 11), which is advisory and gates nothing — #989's idle *wait* was fatal and hung on panes that were already idle, which made it the top failure mode over seven live handoffs.
...
| `/goal` pasted mid-turn produced its row while the turn ran | the guard needs no idle window, so it can go last. #989 marked this FALSIFIED and moved the guard second; that reversal was reverted after the original order was measured working for weeks |
...
The condition itself is read **verbatim** from the predecessor's own newest **trusted** `goal_status` row (`launcher_lib.py read-goal-condition --transcript <file>`), never retyped or summarised. "Trusted" is origin-bound and not merely a field test (#864): the row must sit at the record's TOP-LEVEL `attachment`, carry `sentinel: true` and a literal boolean `met` — so a `goal_status` object forged inside ordinary message content is ignored rather than returned. The command reports one of four states, `LIVE`, `CLEARED`, `NEVER_ARMED` or `AMBIGUOUS`, and exits 0 only for `LIVE`. A guard that a later trusted row satisfied reports `CLEARED`, **not** the condition it used to carry — before #864 this command returned the stale condition, which silently revives a spent merge authorization. **SCOPE, and read it before relying on any of the above: everything in this paragraph describes the `read-goal-condition` CLI ONLY.** The campaign `handoff` and `mid-child-handoff` readers still call the historical `last_unmet_goal_condition`, so on those two paths a forged nested row and a cleared-then-stale condition both remain possible. Origin-binding them is #772 and it is not done yet. For the mid-child path the following is a SEPARATE, older mechanism and not the trusted reader (Step 11): an explicitly supplied `--goal-condition` is checked against the transcript, and a condition that a later `met:true` row has satisfied — or that a replacement guard has superseded — is refused, because arming the successor with it would hand over a guard that is not what is owed and would also re-arm the predecessor wrongly on the partial-success path. The **newest trusted** row wins, because a run can re-arm its goal and only the most recent row states what is still owed — and if that newest row says `met: true`, the answer is `CLEARED`, not the older condition. No trusted row is an explicit refusal (`NEVER_ARMED`), not an invented condition; an unreadable newest row is `AMBIGUOUS`, which is deliberately not reported as a proven "no goal".
...
|---|---|---|---|
| 1 | `queue_revalidated` | `.driver-state`: a `queue_revalidation` receipt whose `validated_head` equals a **freshly observed** `origin/main`, with every **eligible** child stamped at that head (#840). A third clause on `pending_disposition` was cut and is being rebuilt in #848 — a marker gates nothing today | predecessor |
...
| 3 | `project_switched` | `session_registry.jsonl`: ONE line carrying the new session id **and** the recorded project **and** project_path | predecessor |
| 4 | `prompt_landed` | successor transcript: the generation-bound handoff marker, matched as a plain **substring** | predecessor |
| 5 | `goal_armed` | successor transcript: a `goal_status` attachment with `met:false` whose condition is the one actually armed | predecessor |
...
**`queue_revalidated` is FIRST, and its producer ships with it.** The queue must be revalidated before a successor is spawned to inherit it — a successor handed a stale queue has already read the wrong issue bodies by the time a later rung could object. Its result is computed by the launcher reading the durable receipt (`produce_queue_revalidated`), never supplied by the caller: an agent asserting its own homework is exactly the vacuous pass the gate exists to prevent. It is produced at BOTH gate sites — `perform_handoff` via an explicit `campaign_context`, and `retire_predecessor`, which recomputes it because another child may have merged since the predecessor reported. A campaign carrying no receipt **FAILS** the rung. An earlier revision passed it, on the compatibility argument that every pre-#840 campaign has no receipt; the Step-11 review called that opt-in theatre and the owner closed it 2026-08-02. **Every refusal this rung makes ABOUT THE QUEUE — a stale receipt head, an unstamped eligible child — is cleared by `/rawgentic:revalidate-children`**, and the message names it. A refusal about the CALLER or the ENVIRONMENT is not: an absent `campaign_context`, an unreadable state file or an unobservable head are producer failures that no revalidation repairs, and the message distinguishes them. That is true because the owner gate — the one clause the skill could NOT clear — was cut from #840 and is being rebuilt in #848; when it returns, this paragraph grows the second remedy back. `tests/hooks/test_revalidation_jam_matrix.py` drives all 3840 reachable states through their own printed instructions to keep the claim honest either way. Silent passage remains the one failure direction this gate forbids. The migration is therefore real and deliberate: an existing campaign refuses until it has been armed once.

Fail-closed is unchanged from §7.4: an unreported step counts as failed — which is why the rung and its producer had to land in one commit rather than the rung alone. #611's three-step launch ladder is untouched — the mid-child ladder is a separate tuple, and each step carries an `owner`, which is load-bearing rather than documentation: a ladder carrying successor-owned checks forces predecessor-side teardown **off**, so a predecessor can never retire itself after only the five checks it can make. (It was four before #840 added `queue_revalidated` as a predecessor-owned rung; the count is `_MID_CHILD_VERIFICATION_STEPS` entries whose `owner` is `predecessor`, and rows 3–5 of the table above are in production order — `project_switched → prompt_landed → goal_armed`, `hooks/launcher_lib.py:308`; #989 briefly moved the guard ahead of the work and was reverted — and the order is load-bearing, because `evaluate_verifications` stops at the FIRST failure and a ladder out of send order names the wrong step as the cause.)
...
3. **Verify** checks 1–5 — the predecessor-owned rungs — from the successor's own artifacts, then read check 6's `position_rebuilt` receipt. (An earlier revision said "checks 1–4, then the receipt", which predates #840 inserting `queue_revalidated` at position 1 and reads as though the receipt were the queue one. `queue_revalidated` is check 1 and is RECOMPUTED here, not inherited from the predecessor's report — another child may have merged since it reported.)
