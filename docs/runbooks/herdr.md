...
|---|---|---|---|
| 1 | `queue_revalidated` | `.driver-state`: a `queue_revalidation` receipt whose `validated_head` equals a **freshly observed** `origin/main`, with every **eligible** child stamped at that head and no **durably-undisposed** child carrying a `pending_disposition` — the two clauses have different scopes on purpose (#840) | predecessor |
