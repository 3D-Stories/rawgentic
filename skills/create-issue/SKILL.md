...
2. Render it to a self-contained, CSP-safe HTML artifact — never hand-roll HTML. Issue
   specs render with the `spec` template (`--style spec`) per the design language.
   So `<STYLE>` = `spec`, `<MD>` = `<doc>.md`, `<OUT>` = `<doc>.html`, `<TITLE>` = `#<issue> <title>`,
   `<TELEMETRY>` = `` (WF1 has no run-record).
<render-addon>
...
are failures: never treat either as the fallback.
   ...
   On exit `20` say plainly in your output that the add-on is absent and the page is a
   plain fallback. On `70`/`71` stop and report — never present them as the fallback.
