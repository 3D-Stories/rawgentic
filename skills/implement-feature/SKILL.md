...
At workflow start (Step 1), detect project capabilities from two sources:

**Source 1: CLAUDE.md "Project Constants" section** (generated by /rawgentic:setup)
The setup wizard detects and populates constants. Read the `## Project Constants` section to get:
- `REPO` and `PROJECT_ROOT` (always present)
- Optional constants (only present if setup detected them): `DEV_HOST`, `ENGINE_HOST`, `DB_NAME`, `DB_USER`, `POSTGRES_CONTAINER`, `COMPOSE_INFRA`, `COMPOSE_ENGINE`, `DASHBOARD_PORT`, `ENGINE_API_PORT`
- `## Test Commands` section: actual commands or a comment indicating none detected
- `## Deploy Commands` section: actual commands or a comment indicating none detected

Any constant not present in CLAUDE.md is treated as "not available" — do NOT halt the workflow.

**Source 2: Filesystem probing** (supplements CLAUDE.md for dynamic detection)
- `has_ci`: `.github/workflows/*.yml` exists?
- `has_migrations`: `migrations/`, `postgres-migrations/`, `alembic/`, `db/migrate/` exists?
- `has_formatting`: `.prettierrc`, `pyproject.toml` with [tool.ruff], `.eslintrc*` exists?

Combine both sources into a `capabilities` object:

```
capabilities = {
  # Always available (from CLAUDE.md Project Constants)
  REPO: string,
  PROJECT_ROOT: string,

  # From CLAUDE.md (optional — null if not set by /rawgentic:setup)
  DEV_HOST: string | null,
  ENGINE_HOST: string | null,
  DB_NAME: string | null,
  DB_USER: string | null,
  POSTGRES_CONTAINER: string | null,
  COMPOSE_INFRA: string | null,
  COMPOSE_ENGINE: string | null,
  DASHBOARD_PORT: string | null,
  ENGINE_API_PORT: string | null,

  # Derived from CLAUDE.md + filesystem
  has_tests: boolean,
  test_commands: string[],        # from CLAUDE.md "Test Commands" section
  has_ci: boolean,
  has_deploy: boolean,
  deploy_method: string | null,   # "script" | "ssh" | "compose" | "manual" | null
  infrastructure: {
    hosts: [{name, address, ssh_alias, description}],
    containers: string[],
  ...
  project_type: string,           # "application" | "infrastructure" | "scripts" | "library" | "docs"
}
```

**Project type classification heuristics:**
- `application`: has package.json with start script, or main entry point, or src/ directory with app code
- `infrastructure`: primarily Docker Compose files, Terraform, Ansible, or deployment configs on remote servers
- `scripts`: primarily standalone scripts (Python, Bash) with no framework structure
- `library`: has package.json with main/exports, or setup.py/pyproject.toml with [project] section
- `docs`: primarily Markdown, with publishing scripts or static site generator

When a repo could match multiple types (e.g., docs with publishing scripts), classify based on what the current task produces: if the task creates/modifies a script, use `scripts`; if it creates/modifies infrastructure configs, use `infrastructure`; if it creates/modifies documentation content, use `docs`. The task's output type takes priority over the repo's overall character.

The project type influences how "implementation" and "testing" are interpreted throughout the workflow. When the project type is `infrastructure` or `scripts`, "testing" means verification scripts and health checks, not unit tests. When the project type is `docs`, "testing" means content validation and link checks.
</capability-detection>
...
1. **Detect project capabilities** by reading CLAUDE.md, checking the filesystem, and running discovery commands. Populate the `capabilities` object per `<capability-detection>`. Log all detected capabilities in session notes.

   Detection commands:
   - `REPO`: `git remote get-url origin | sed 's|.*github.com[:/]||;s|\.git$||'`
   - `PROJECT_ROOT`: `git rev-parse --show-toplevel`
   - `has_tests`: Check CLAUDE.md "Test Commands" section; check for `tests/`, `__tests__/`, `spec/`, `*.test.*` files
   - `has_ci`: Check `.github/workflows/*.yml` existence
   - `has_deploy`: Check CLAUDE.md "Deploy Commands" section; check `scripts/deploy*`
   - `infrastructure`: Read CLAUDE.md infrastructure section; check for SSH aliases, Docker compose refs
   - `has_migrations`: Check for `migrations/`, `postgres-migrations/`, `alembic/`, `db/migrate/`
   - `project_type`: Apply heuristics from `<capability-detection>`

   For any capability that cannot be detected, set to `false` / `null` — do NOT halt the workflow.
...
3. **Live environment probe (infrastructure projects only):** When `project_type == "infrastructure"` and target hosts are known (from `capabilities.infrastructure.hosts`), SSH to each target host to discover current state. This catches discrepancies between issue specs (which may be outdated) and reality.
...
1. Launch three judge sub-agents in parallel. If any returns 429, retry that agent after 30s.
   ...
   **Judge 1: Architecture & Patterns Reviewer**
   - Does the design respect existing patterns documented in CLAUDE.md?
   - Is the architecture consistent with project conventions?
   - Are dependencies appropriate (prefer libraries per CLAUDE.md Library-First approach)?
...
2. Pull latest main and create branch:
   ```bash
...
2. **Dispatch 3-agent parallel review.** If any returns 429, retry that agent after 30s.
   ...
   **Agent 1: Style & Convention Compliance**
   - Code style rules from CLAUDE.md (whatever rules are documented there)
   - Naming conventions
   - Import ordering
...
3. **Deploy (adaptive based on capabilities.deploy_method):**
   ...
   **If `deploy_method == "script"`:**
   Run the deploy script from CLAUDE.md Deploy Commands section.

   **If `deploy_method == "ssh"`:**
   SSH to infrastructure hosts from `capabilities.infrastructure.hosts` and execute the deployment commands appropriate for the change (docker compose up, service restart, config reload, etc.). Generate commands based on the implementation plan — do NOT use hardcoded commands.
