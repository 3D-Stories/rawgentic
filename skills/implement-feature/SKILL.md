...
```
**A `review`-seat dispatch whose brief references attached material MUST pass that material as `--context-file` (#826).** The flag is bracketed because most briefs inline everything they need; the moment a brief says "the attached design", "the diff below", or "supplied as context", the artifact is a REQUIRED input. `dispatch_seat` refuses that combination pre-receipt with `review_context_required` (exit 2) — measured at `3a017dd9`, 10 recorded review dispatches asked for a review of material they never attached, and each returned a verdict the gate read as a pass.
