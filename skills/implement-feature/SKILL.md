...
3. **Deploy to my-api-dev:** The PostToolUse deploy hook triggers `scripts/deploy-dev.sh` which:
   - SSHs to my-api-dev (${DEV_HOST}), pulls latest main, restarts containers
...
- Deploy fails on my-api-dev: check deploy script logs, SSH and inspect container logs
...
Deployment:
- my-api-dev (${DEV_HOST}): [success / failed]
