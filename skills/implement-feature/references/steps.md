...
1. **Before dispatch:** record the pre-task state — current `HEAD`, the checked-out symbolic feature ref (`git symbolic-ref -q HEAD`; a detached HEAD REFUSES this mode — there is no feature ref to land on, #767), and `git status --porcelain` (the tree must already be clean from the previous task's commit). The recorded ref and SHA are what item 3's landing asserts against.
