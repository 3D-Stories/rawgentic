...
2. **Dispatch one implementation task-agent** — executor `build` seat under the default architecture; under the declared LEGACY architecture only, a `rawgentic:rawgentic-implementer` via the Agent tool (serial — one at a time; each task builds on the previous commit) with the per-task `model` from item 0 and the brief: the design doc, this plan task, the TDD requirement, project conventions, and the current test baseline. The definition carries the test-first/stage-only-your-files/report-honestly contract; the agent implements the task test-first and commits it. Its `isolation: worktree` frontmatter keeps the mutation off the shared tree (fallback per `<model-routing-resolve>` when worktrees are unavailable).
