...
Dispatch every Step 2 fan-out as a concurrent executor dispatch of the `analysis` seat per the `<model-routing-resolve>` contract (primary tier). Same briefs, same synthesis barrier as before — only the dispatch mechanism changes. The fallback (legacy) tier dispatches the same briefs as generic analysis subagents via the Agent tool (no bundled analysis agent; `model: <analysis>` unless `inherit`; effort dual-path, always logged). Each DISPATCH line's producer is the executor result dict (`resolution=primary`) or the fallback-tier subagent (`resolution=fallback`).
...
design gate is a strong-model surface. The fallback (legacy) tier dispatches it as a
   ...
   - If the merged list contains one or more Critical/High findings, fold their
     ...
     **Verifier-brief hardening (#407):** when a spec_tighten cheap pass was reached via
     ...
     - Fold = `spec_tighten` → the **spec-tightening cheap path** (#223): consume
       ...
       (`<model-routing-resolve>` primary tier; fallback tier: a `rawgentic:rawgentic-reviewer`
...
<!-- model-routing: role=review -->
Dispatch the 2 review agents on the executor `review`/`review_fast` seats per the `<model-routing-resolve>` contract (primary tier); the fallback (legacy) tier dispatches them as `rawgentic:rawgentic-reviewer` via the Agent tool (`model: <review>` unless `inherit`; effort dual-path, always logged; `resolution=fallback`). Per `<review-lens-routing>` (SKILL.md): Reviewer 1 → `mechanical` (the bug_logic brief folded in, fast tier), Reviewer 2 → `security` (strong); resolve each via `resolve --role review --lens <lens>`; an `inherit` resolution on a Haiku session dispatches `model: sonnet` (never-Haiku). Every reviewer brief MUST restate the read-only execution clause (#510): Bash is for read-heavy inspection only — never execute the target project's entry-point scripts, deploy paths, or anything that mutates state or sends outward; the only sanctioned executions are the verification commands this brief names (from the project's `.rawgentic.json` testing config); an entry script invoked in an unexpected form may fall through to a live path — do not experiment with invocation forms; when a command's read-only-ness is uncertain, don't run it — report the uncertainty as part of the review.
