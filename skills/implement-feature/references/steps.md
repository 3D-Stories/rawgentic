...
design gate is a strong-model surface. **The design document is a REQUIRED input: pass it as `--context-file <design-doc>` (#826).** A brief that says "review the attached design" with no context file is refused `review_context_required` at the dispatch choke-point — a review of nothing still returns a verdict this gate would read as a pass. Under the LEGACY architecture (declared, `defaultArchitecture: legacy`) it dispatches as a
...
<!-- model-routing: role=review -->
Dispatch these reviewers on the executor `review` seat per the `<model-routing-resolve>` contract (primary tier; the fast tier is a #491 lens/model choice, not a seat). **The concatenated high-risk diff is a REQUIRED input: pass it as `--context-file <diff>` (#826)** — a brief referencing an attached diff with no context file is refused `review_context_required` at the dispatch choke-point. Under the LEGACY architecture (declared, `defaultArchitecture: legacy`) they dispatch as `rawgentic:rawgentic-reviewer` agents via the Agent tool (`model: <review>` unless `inherit`; effort dual-path, always logged; `resolution=fallback`). Every reviewer brief MUST restate the read-only execution clause (#510): Bash is for read-heavy inspection only — never execute the target project's entry-point scripts, deploy paths, or anything that mutates state or sends outward; the only sanctioned executions are the verification commands this brief names (from the project's `.rawgentic.json` testing config); an entry script invoked in an unexpected form may fall through to a live path — do not experiment with invocation forms; when a command's read-only-ness is uncertain, don't run it — report the uncertainty as part of the review.
...
<!-- model-routing: role=review -->
Dispatch the 2 review agents on the executor `review` seat per the `<model-routing-resolve>` contract (primary tier; the fast tier is a #491 lens/model choice, not a seat). **The PR diff is a REQUIRED input: pass it as `--context-file <diff>` (#826)** — a brief referencing "the diff below" with no context file is refused `review_context_required` at the dispatch choke-point, because a review of nothing still returns a verdict this gate would read as a pass. Under the LEGACY architecture (declared, `defaultArchitecture: legacy`) they dispatch as `rawgentic:rawgentic-reviewer` via the Agent tool (`model: <review>` unless `inherit`; effort dual-path, always logged; `resolution=fallback`). Per `<review-lens-routing>` (SKILL.md): Reviewer 1 → `mechanical` (the bug_logic brief folded in, fast tier), Reviewer 2 → `security` (strong); resolve each via `resolve --role review --lens <lens>`; an `inherit` resolution on a Haiku session dispatches `model: sonnet` (never-Haiku). Every reviewer brief MUST restate the read-only execution clause (#510): Bash is for read-heavy inspection only — never execute the target project's entry-point scripts, deploy paths, or anything that mutates state or sends outward; the only sanctioned executions are the verification commands this brief names (from the project's `.rawgentic.json` testing config); an entry script invoked in an unexpected form may fall through to a live path — do not experiment with invocation forms; when a command's read-only-ness is uncertain, don't run it — report the uncertainty as part of the review.
