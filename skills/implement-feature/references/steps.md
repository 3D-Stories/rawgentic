...
1a. **Adversarial diff review sub-step (opt-in, cross-model — runs concurrently with the 3 review agents; issue #131).**
   ...
   - **Stale sweep (first thing):** delete any leftover `.rawgentic-diff-review-*.patch` and `.rawgentic-diff-findings-*.json` (including `-glm` siblings) under the project root before doing anything else. This is crash recovery — a finally-style cleanup-on-exit cannot cover a SIGKILL, so a prior run's stale temp files may still be on disk.
