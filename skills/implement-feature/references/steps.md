...
4. **Dispatch ONE build-subagent** — executor `build` seat under the default architecture; under the declared LEGACY architecture only, `rawgentic:rawgentic-implementer` via the Agent tool — with the brief (design, plan w/ riskLevels, TDD requirement, conventions, baseline) + the required receipt schema (both in the reference file). Model: `select_impl_model(<ceiling>, <riskLevel>, <complexity>)` sized to the plan's **highest-risk** task (one agent → size to the hardest task); **never `model: haiku`** (same inherit+Haiku→sonnet rule as above).
