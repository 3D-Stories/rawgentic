...
5. **Render ONE consolidated pair** —
   ...
   via the add-on renderer with `--style report` (same-day rerun suffixes apply): an
...
HTML via the add-on's renderer — never hand-rolled. WF14 reports render with the
`report` template (`--style report`) per the design language (`docs/design-language.md`).
So `<STYLE>` = `report`, `<MD>` = `<report>.md`, `<OUT>` = `<report>.html`,
`<TITLE>` = `WF14 run-feedback — WF<n> #<issue>`, `<TELEMETRY>` = ``.
<render-addon>
