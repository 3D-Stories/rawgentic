...
2. **Check for GitHub PAT in project CLAUDE.md.** Search for `github_pat_` pattern.
   - **If found:** Tell the user: "WARNING: The project CLAUDE.md contains a GitHub PAT. This is a security concern if this file is committed to git. The PAT should be in your workspace CLAUDE.md (`{WORKSPACE_ROOT}/CLAUDE.md`) or personal CLAUDE.md (`~/.claude/CLAUDE.md`). Would you like me to remove it from the project file?" Only remove if user approves.

3. **Check for team process sections.** Search for patterns: `## SDLC`, `## Workflow Principles`, `## Project Constants (generated by`, `## Test Commands (detected by`.
...
4. **Check for duplicate hooks.** If `<path>/.claude/` exists, list any hook files and compare names against `{WORKSPACE_ROOT}/.claude/hooks/` (if it exists).
