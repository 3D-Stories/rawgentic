...
- **Step 4 - Quality gate: lightweight reflect.** the quality-bar rubric (`references/quality-bar.md`) on RCA correctness; opt-in default-off cross-model adversarial review; the breaker runs on the merged findings. (read references/steps.md before executing)
