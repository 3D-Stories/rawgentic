...
```
**A `review`-seat dispatch whose brief references attached material MUST pass that material as `--context-file` (#826).** The flag is bracketed because most briefs legitimately inline everything they need — a brief that says the diff is "inlined below" is self-contained and is NOT refused. Emit the machine-readable marker `<!-- rawgentic:requires-context -->` in the brief when the artifact really is external: that marker is the PRIMARY signal, immune to rewording, and a brief saying "the attached diff" is the prose backstop, because a caller who forgets the attachment can equally forget the marker. The dispatch refuses pre-receipt with `review_context_required` (exit 2) — and an empty or whitespace-only context file does not count as attached.
