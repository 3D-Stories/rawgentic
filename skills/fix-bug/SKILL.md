...
```
Emission rules:
- One line per SUBAGENT INVOCATION dispatched (not per attempt) — WF3 Step 9's two review agents = two lines at a single tier; a slot that falls back adds the abandoned tier's terminal line.
- A slot that falls back emits TWO lines: the abandoned tier's terminal line (`outcome=error resolution=primary` for an abandoned tier 1 — the primary path was attempted and errored) plus the new tier's line. A double-unavailability (tiers 1 AND 2 unresolvable → tier 3) emits one abandoned-terminal line per abandoned tier.
