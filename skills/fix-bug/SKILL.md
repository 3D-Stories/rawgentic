...
```
**A `review`-seat dispatch whose brief references attached material MUST pass that material as `--context-file` (#826).** The flag is bracketed because most briefs inline everything they need; the moment a brief says "the attached diff" or "the diff below", the artifact is a REQUIRED input, and `dispatch_seat` refuses the combination pre-receipt with `review_context_required` (exit 2).
