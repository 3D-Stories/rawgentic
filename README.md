...
### v3.24.8 (2026-07-08)
- **One jq parse of hook stdin per WAL event (#266, epic #278).** `wal_parse_fields()` spawned four separate jq processes per tool call (the WAL hot path's dominant cost, review finding C8+C11); it now runs ONE jq that emits all four field assignments through jq's `@sh` filter, applied with `eval` — every value arrives single-quoted, so quotes, newlines, and command substitution in field values stay inert (test-enforced, including a marker-file injection probe). Defaults are pre-set so malformed stdin yields the same sentinels as missing fields (`unknown`/`.`) instead of accidental empty strings; `set -e` exit semantics match the old code path. All 7 WAL hooks route through the one function; red-before-green spawn-count (4→1) and malformed-defaults tests; a set-e caller-contract pin added from the convergent 8a review note. No workflow-spine change → no diagram REV. Suite 2336→2342.
