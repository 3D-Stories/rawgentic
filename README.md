...
### v3.24.10 (2026-07-07)
- **wal-bind-guard: bound-project fast path (#268, epic #278).** A bound session touching its OWN project's files — the common case on every Read/Edit — now short-circuits with a bash prefix test on the registry's absolutized `project_path`, before the workspace-root subshell and the cross-project violation jq scan (6 jq spawns → 5, red-before-green PATH-shim count test). Unbound sessions, other-project files, and the cross-project allowlist are byte-identical; a registry entry without `project_path` falls through to the unchanged scan. Enabler fixed in wal-lib: `wal_resolve_project` now strips a leading `./` when absolutizing (the old `/./`-embedded form was an accidental artifact one test had pinned; it broke literal prefix comparison). Named delta: nested project dirs (unsupported, none exist) would fast-path-allow a file under both the bound and a nested project where the scan denied. No workflow-spine change → no diagram REV. Suite 2347→2349.
