...
### v3.66.4 (2026-07-19)
- **Capture dirs chmod 0700 — the transport envelope stops being world-readable (#513).** `phase_executor`'s `create_capture` made attempt/run/seat dirs with default umask (typically 0755) while the W4 supervisor's specs/registry dirs are explicitly 0700 — and captures hold `transport.stdout.txt`, the raw `claude --output-format json` envelope including the provider `session_id`, readable by any local user on a shared host (surfaced by the #467 Step-11 R2 review; pre-existing since the #424 capture layer). Every dir the call CREATES is now chmod 0700 after the mkdir (the supervisor pattern — `mode=` is umask-masked, chmod is not); pre-existing intermediates and the caller-owned root keep their mode (posture set at creation time). Two red-first mode tests pin both directions (fresh tree all-0700 under a 0o022 umask; a pre-existing 0755 run dir not re-chmodded). No workflow-spine change → no diagram REV. Suite 3800+10skip→3802+10skip.
### v3.66.3 (2026-07-19)
